...
class TradePassword extends Component {
    ...
    componentDidMount() {
        ...
        dispatch(authCode())
        this.setState({
            username:this.props.personal.personal.userName||this.props.detail.myInfo.userName,
        })
      }
    handleChange (type, e) {
        this.setState({
            ...
            <div className='retrievePassword-body'>
               <form className='retrievePassword-form'>
                    <div className='retrievePassword-box retrievePassword-name-box'>
                        <label>验证码</label>
                        <input type='text' className='retrievePassword-password' placeholder='请输入短信验证码' onChange={this.handleChange.bind(this, 'trade_password_code')} value={this.state.trade_password_code}/>
