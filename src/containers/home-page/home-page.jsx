    ...
    return (
      <div className="home-page home-body" id="home-page">
        <div className="home-content footer-tab-content">
          {!isWeiXin() ? <div className="page-title">首页</div> : null}
          <div className="banner">
            <ReactSwipe
              swipeOptions={swiperOptions}
              showPagination
              className="swiper"
              key={home.adverts.carouselList.length}
            >
              {home.adverts.carouselList.map(item => {
                return (
                  <img
                    src={item.imgsrc}
                    key={item.id}
                    onClick={this.handleBannerClick.bind(this, item.imgurl)}
                  />
                );
              })}
            </ReactSwipe>
          ...
          <ul className="other-list">
            {home.adverts.randomList.map(item => {
              return (
                <li
                  key={item.id}
                  onClick={this.handleAdClick.bind(this, item.imgurl)}
                >
          ...
          <div className="greenhand">
            {home.project.advertList.length ? (
              <img
                src={home.project.advertList[0].imgsrc}
                    ...
                    >
                      ...
                      <div className="yield">
                        <div className="money l">
                          预计年化收益率&nbsp;<span className="number">
                            {item.annualRate}
                            {item.raiseRate ? `+${item.raiseRate}` : ""}
                          </span>
                          <span className="unit">%</span>
                        </div>
                        <div className="deadline r">
                          期限&nbsp;<span className="number">
                            {item.loanExpiry}
                          </span>
